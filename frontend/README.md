...
| `/security`                             | `pages/SecurityPage.tsx`       | —                       | 二要素認証とパスキーの登録・解除、IdP との連携（ADR-0036）                              |
...
| 招待リンクの表示        | 発行直後だけ出る。`{origin}/join#code=...`（`services/invitationLink.ts`）。コードも併記する | —                                               |
| 「リンクをコピー」      | 招待リンクをクリップボードへ写す。触れない出所では失敗を伝える                               | —                                               |
| 有効な招待の表          | 対象・有効期限・「取り消す」                                                                 | `GET` / `DELETE /api/families/{id}/invitations` |

一覧にコードは載らない（`code` は常に `null`）。失くしたら発行し直す。

招待リンクの出所は、発行した画面の `window.location.origin`。設定の `APP_BASE_URL`
（メール本文の生成元）は既定が空のため使わない。受け取った人が開くと `/join` の
入力欄にコードが入った状態から始まり、そこからログインへ回っても
コードは `/families` まで運ばれる。

### 招待コードは URL の断片で運ぶ（ADR-0025）

コードを載せる場所は `#code=...` で、クエリ（`?code=`）には置かない。断片は
ブラウザがサーバーへ送らないため、リバースプロキシのアクセスログにもリクエスト
ログにも平文が残らない（コードは有効な間そのまま家族へ入れる値で、DB には
ハッシュしか置いていない）。

組み立てと読み取りは `services/invitationLink.ts` に集約する。受け取り側の 3 画面は
どれも `useLocation().hash` から読む（`useSearchParams` は使わない）。

| 画面        | 実装                       | コードの使い道                                      |
| ----------- | -------------------------- | --------------------------------------------------- |
| `/join`     | `RedeemInvitationPage.tsx` | 入力欄の初期値。ログインへ回すときも持たせる        |
| `/login`    | `LoginPage.tsx`            | 先にログインする理由を出し、後で `/families` へ送る |
| `/families` | `FamiliesPage.tsx`         | 参加の欄の初期値。使い終えたら URL から消す         |

1 か所でもクエリへ戻すと、そこから先が平文でログに載る。回帰は
`services/invitationLink.test.ts`（`?` を含まないこと）が見る。
...
| 「このポイントを読み込めませんでした。」 | 取得に失敗し、まだ何も出せていない                       | 見えない台帳（404）を直接開いた場合を含む                              |
...
### 残高を読み直すきっかけ（ADR-0021）

残高はダッシュボード・ナビゲーション・家族の詳細にも出る。これらは家族の詳細
応答（`store/FamilyContext`）を共有しており、台帳の応答とは別物なので、次の
きっかけで読み直す。

| きっかけ                                         | 読み直すもの                 |
| ------------------------------------------------ | ---------------------------- |
| 加算・消費・取り消しが成功した                   | 台帳と家族の詳細             |
| 画面を移った（SPA 内の遷移）                     | 家族の詳細                   |
| 画面が手元に戻った（タブの再表示・圏外復帰）     | 家族の詳細と、開いている台帳 |
| 家族の内容を変えた（子の追加・招待・並べ替え等） | 家族の詳細                   |

家族の詳細を**ログイン中に 1 回だけ**取る作りにはしない。1 回きりだと、別の端末・
別のタブで足されたポイントがブラウザの再読込まで出てこない。

裏に回っているあいだは読み直さない。記録に失敗したときも読み直さない。
読み直しでは表示を「読み込み中」に戻さず、値が届いたときに入れ替えるだけにする
（画面を移るたびに空になるのを避けるため）。読み直しに失敗しても、すでに出している
残高・履歴は消さず、エラーはトーストで知らせる（どの時点の値かは取得時刻の行が
示す）。先に始めた取得が後から届いても、新しい内容は上書きしない。
