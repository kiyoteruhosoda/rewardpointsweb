...
家族の詳細の中に埋め込まれる。親（`owner` / `parent`）以外には現れない。
...
| コード                            | 表示の意味                                             |
| --------------------------------- | ------------------------------------------------------ |
| `family_not_found`                | 存在しないか、自分が所属していない                     |
| `family_access_denied`            | 所属はしているが、その立場では行えない                 |
| `ledger_not_found`                | 存在しないか、自分には見えない（兄弟の台帳を含む）     |
