...
- 相手は**すでに登録済みで有効なアカウント**であること。未登録・無効なら
  `share_target_not_found` になる。招待メールは送らない — 相手が `member:view` を
  持っていれば、次にログインしたときそのメンバーが `/members` に並ぶ。持っていない
  アカウント（既定では `guest`）へも共有そのものは成功するが、Sidebar に「ポイント」は
  出ず、API も 403 を返す。**共有は scope を与えない。**
...
下の表は**メンバー単位アクセス**の面だけを示す。実際に見え・行えるかは、列ごとの
scope も併せて必要（二段。ADR-0007）。

| 立場                     | `/members` に並ぶ | 残高・履歴   | 加算・消費     | メンバー削除    | 共有の管理      |
| ------------------------ | ----------------- | ------------ | -------------- | --------------- | --------------- |
| 併せて必要な scope       | `member:view`     | `point:view` | `point:manage` | `member:manage` | `member:manage` |
| 所有者                   | ○                 | ○            | ○              | ○               | ○               |
| `manage` で共有された人  | ○                 | ○            | ○              | ×               | ×               |
| `view` で共有された人    | ○                 | ○            | ×              | ×               | ×               |
...
`member:view` + `point:view` だけ、`guest` はどれも持たない。つまり `manage` で
共有しても、相手が `point:manage` を持たなければポイントは記録できない。本人
（`member` ロール）が変更できないのは、この scope と、本人が `view` 止まりである
関係との両方による。
