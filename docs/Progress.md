...
| 2 | T2 | 「止めたら届く」の残り（定期照合と、assay 側で停止時にも送ること） | ⬜未着手 | 大 | 中 |
...
idp の ADR-0049 I7 の 3 つのうち、受け口は ADR-0032 で、短命のアクセストークンは
ADR-0037 で入った（**5 分**）。⚠ **ここが「止めたら効くまで」の上限時間である。**

残りは **定期照合**。⚠ **本体は assay 側である** ——いまの assay は「利用者を止めた」
ときに通知を送らない（送るのはログアウトのときだけ）。
