...
⚠ **入れる前に、assay 側の認証ポリシーを先に用意すること。** `acr_values` は assay に
とって**ポリシーの条件**（`requested_acr`）でしかなく、**それだけでは強度は上がらない。**
対応するポリシー（条件 `requested_acr = urn:assay:ac:mfa` → 効果 MFA 要求）が無いと、
assay はいつもどおり認証して**実際に使われた強度をそのまま返す**。その結果:
