...
- **セーフエリアを避ける**。`viewport-fit=cover` で画面いっぱいに描き、ノッチ・
  ホームインジケータの領域は `env(safe-area-inset-*)` で個別に空ける。
...
- **アプリアイコンを RewardPoints のものにした**。テンプレート由来の「F」から、
  金貨（貯まるポイント）の中で大きなハート（親）が小さなハート（子）を包む、
  左右対称の意匠へ差し替えた。5 ファイルは `scripts/generate_app_icons.py` が
  SVG・PNG とも同じ座標から生成する（手で編集しない）。ランチャー側で切り抜かれる
  `maskable` は、角丸のアイコンを流用せず、端まで塗って図柄をセーフゾーン（内側 80%）に
  収めた専用画像（`pwa-maskable-512x512.png`）を用意した。
