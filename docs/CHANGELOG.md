...
- 参加者ごとの操作は、家族の詳細応答が返す `can_reset_password` /
  `can_graduate` / `can_remove` だけで出す。判定はドメインの
  `family_access_policy` にあり、実際に断る条件と同じものを見る（台帳の
  `can_modify` と同じ考え方）。
- 子を家族から外す道を画面で 2 つに分けた。**卒業**（ADR-0014 の独立 —
  アカウントのある子。本人の承認で成立し記録も消える）と、**削除**（記録の無い
  参加者だけ。子ならアカウントも消える）。「独立」は画面上の呼び名を「卒業」に
  改めた（API・DB の語彙は独立のまま）。
